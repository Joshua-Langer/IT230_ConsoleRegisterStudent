...
namespace ConsoleRegisterStudent
{
    class Program
    {
        ...
        void run()
        {
            ...
            System.Console.WriteLine("Joshua Langers's Copy"); //Took ownership of this copy.
        ...
        {
            if (choice < 1 || choice > 7) //Removed 70 and set to 7 for or option to prevent incorrect casing from occurring.
                return -1;
            else if (choice == firstChoice || choice == secondChoice || choice == thirdChoice) //changed from && to || so we can properly access the case.
                return -2;
            else if (totalCredit >= 9) //Added =
                return -3;
            return 0; //0 instead of -4, to match the case rule in run.
        }
